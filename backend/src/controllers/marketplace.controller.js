...
export const update = async (req, res) => {
  const { id } = req.params;
  const { title, description, category, type, price, location, status } = req.body;
...
export const interest = async (req, res) => {
  ...
  try {
    ...
    if (item.user_id === req.user.id) return res.status(400).json({ error: 'Não podes mostrar interesse no teu próprio item' });
    if (item.status !== 'disponivel') return res.status(400).json({ error: 'Item já não está disponível' });
