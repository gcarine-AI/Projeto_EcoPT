...
export const calculate = async (req, res) => {
  ...
  if (km <= 0 || passengers <= 0) {
    return res
      .status(400)
      .json({ error: "km e passengers devem ser maiores que 0" });
  }
...
export const bookRide = async (req, res) => {
  ...
  try {
    ...
    const saved_co2 = ride.km ? Math.round(ride.km * 0.21 * 100) / 100 : null;
    ...
    if (bookingError)
      console.error("Erro ao guardar reserva:", bookingError.message);

    return res
      .status(200)
      .json({ message: "Lugar reservado com sucesso!", saved_co2 });
  } catch (error) {
    console.error("Erro na reserva:", error);
    return res
      .status(500)
      .json({ error: "Erro ao atualizar reserva no servidor" });
  }
};
